...
print("--socket path---")
print(package.searchpath("socket", path))
print("--tmpsocket---")
local tmpsocket = require("socket")
print( tmpsocket)
print("--------------------------------")
...
local function create_connection(parameters)
    ...
    local perform_connection, tcpsocket

    if parameters.scheme == 'unix' then
        perform_connection, tcpsocket = connect_unix, require('socket.unix')
        assert(tcpsocket, 'your build of LuaSocket does not support UNIX domain sockets')
    else
        if parameters.scheme then
            local scheme = parameters.scheme
            assert(scheme == 'redis' or scheme == 'tcp', 'invalid scheme: '..scheme)
        end
        perform_connection, tcpsocket = connect_tcp, require('socket').tcp
    end

    return perform_connection(tcpsocket(), parameters)
end
...
--lock
function redis:redis_lock(name)
    local isLock = false
    while(isLock ~= true)
    do
        isLock = self:setnx(name, 2)
    end
    return
end

--unlock
function redis:redis_unlock(name)
    self:del(name)
end
...
redis.commands = {
    ...
    hscan            = command('HSCAN', {     -- >= 2.0
        response = function(reply, command, ...)

            local nRes = 0
            local tRes = {}
        
            for key, val in ipairs(reply) do
