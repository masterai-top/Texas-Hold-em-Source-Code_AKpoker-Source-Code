[简体中文](README.md) | [繁體中文](README.zh-TW.md) | [English](README.en.md)

# Texas Holdem System with Short Deck and Insurance|KKpoker|AApoker

## Enterprise-Grade Texas Hold'em Source Code | Multiplayer Competition | Club System | League Tournaments


[![GitHub stars](https://img.shields.io/github/stars/masterai-top/Texas-Hold-em-Source-Code)](https://github.com/masterai-top/Texas-Hold-em-Source-Code/stargazers)
[![GitHub forks](https://img.shields.io/github/forks/masterai-top/Texas-Hold-em-Source-Code)](https://github.com/masterai-top/Texas-Hold-em-Source-Code/network)
[![License](https://img.shields.io/badge/license-Commercial-blue.svg)](LICENSE)


> **Proven track record of stable online operation | Supports leagues, clubs, and private games | Comparable to hhpoker and wpk | Includes source code, art assets, and DevOps scripts**[reference:6]


---


## 📖 Table of Contents


- [Project Overview](#project-overview)
- [Core Features](#core-features)
- [Technical Architecture](#technical-architecture)
- [Quick Start](#quick-start)
- [Feature Showcase](#feature-showcase)
- [Why Choose Us](#why-choose-us)
- [Deliverables](#deliverables)
- [Contact Us](#contact-us)


---


## Project Overview


This is a comprehensive Texas Hold'em solution that has been **successfully deployed and battle-tested in live operations for years**[reference:7]. Unlike the cobbled-together demos often found on the market, our codebase undergoes continuous iteration and ensures stable service;
it is already being used by multiple clubs for real-world operations.
Platforms such as AKpoker and KKpoker utilize our source code;


**Use Cases:**
- 🏢 Building a proprietary Texas Hold'em platform
- 🎯 Developing competitive systems for clubs and leagues
- 📱 Launching card and board games on iOS/Android app stores
- 🛠️ Learning enterprise-grade C++/Cocos game development


---


## ✨ Core Features


| Module | Feature Description |
|------|----------|
| **Lobby System** | Entry points for multiple game modes, announcements, leaderboards, in-game store |
| **Private Games/Clubs** | Private games with friends, club creation/management, league tournaments |
| **Table Logic** | Standard Texas Hold'em / Short Deck / Omaha; auto buy-in, straddle, insurance |
| **Tournament System** | MTT (Multi-Table Tournament), SNG (Sit & Go) |
| **Backend Management** | Player management, statistical reporting, game score adjustments, risk control |


## 🎮 Comprehensive Game Matrix


- **Texas Hold'em** — Classic gameplay
- **Omaha** — Four hole cards, offering more strategic variety
- **Short Deck** — Faster pace, distinct strategy
- **AOF** (All-in or Fold) — Thrilling all-in gameplay
- **MTT** (Multi-Table Tournament) — Large-scale competition
- **SNG** (Sit & Go) — Quick-start tournaments


---


## 🏗️ Technical Architecture


┌─────────────────────────────────────────────────────────────┐
│ Technical Architecture Overview │
├─────────────────────────────────────────────────────────────┤
│ ┌─────────────┐ ┌─────────────┐ ┌─────────────┐ │
│ │ Client-side │ │ Server-side │ │ Data Layer │ │
│ │ Cocos/Unity │◀──▶│ C++ High Concurrency │◀──▶│ MySQL+Redis │ │
│ └─────────────┘ └─────────────┘ └─────────────┘
│
│ │ │ │ │
│ ▼ ▼ ▼ │
│ ┌─────────────┐ ┌─────────────┐ ┌─────────────┐ │
│ │ H5/Web Client │ │ Tars/Private Protocol │ │ Data Persistence │ │
│ │ Cross-platform Support │ │ High-efficiency Protocol │ │ Cache Acceleration │ │
│ └─────────────┘ └─────────────┘ └─────────────┘ │
└─────────────────────────────────────────────────────────────┘


Client: Cocos + JS
Server: C++ and Lua
Admin Backend: Built using the vue-admin-gin open-source framework.
         vue3写前端页面
Go-based Backend Management Server
## 🏆 Why Choose Us?
1. **Battle-Tested, Stable, and Reliable**
This source code has been in active operation across multiple clubs for years, ensuring stable service and continuous code iteration.


2. **Comprehensive Features, Top-Tier Standards**
Outperforms market leaders like HHPoker and WPK in terms of functionality, stability, and scalability.


3. **High-Performance Server**
Server-side written in C++, supporting thousands of concurrent users with ease.


4. **Complete Delivery, Ready-to-Use**
✅ Full server-side and client-side source code


✅ Complete database scripts


✅ HD art assets and UI source files


✅ Deployment and maintenance scripts/documentation


5. **Extensible for Custom Development**
Clear code structure and modular design support customization and feature expansion.

### 📦 Delivery Contents
Delivery Checklist
├── Server-side Source Code (C++)
│   ├── Core game logic
│   ├── Network communication module
│   └── Backend management APIs
├── Client-side Source Code (Cocos Creator / Unity)
│   ├── Full UI/UX implementation
│   └── Multi-platform support
├── Database Scripts (MySQL)
│   ├── Table schemas
│   └── Initial data
├── Art Assets
│   ├── HD images
│   ├── UI source files
│   └── Sound effect files
└── Operations & Maintenance Documentation
    ├── 部署指南
    └── 运维脚本[reference:24]
### 🖼️ Feature Showcase
<img width="246" height="381" alt="主界面" src="https://github.com/user-attachments/assets/d0b2f8fd-8436-4767-998b-c5b55cde276f" />
<img width="246" height="381" alt="账单界面" src="https://github.com/user-attachments/assets/95706dcc-a864-43b5-9141-990a927472aa" />
<img width="246" height="381" alt="游戏界面" src="https://github.com/user-attachments/assets/bce93909-e79f-46c2-8a2e-6fa97ddb75a9" />
<img width="246" height="381" alt="游戏房间界面" src="https://github.com/user-attachments/assets/3d06669e-a7c2-4c73-bafb-433ca8ce712c" />
<img width="956" height="457" alt="系统管理后台界面" src="https://github.com/user-attachments/assets/047ad980-a99f-453b-b1e7-38cbca9b4b48" />
<img width="246" height="381" alt="我的界面" src="https://github.com/user-attachments/assets/9729f825-79d8-4c3a-a931-9a04ac92b0ee" />
<img width="246" height="381" alt="提币界面" src="https://github.com/user-attachments/assets/6e5aa6bc-57f3-4eab-8653-68c9a91fda01" />
<img width="246" height="381" alt="历史保险界面" src="https://github.com/user-attachments/assets/f9c78301-c6c2-4118-a4d0-428ea22c4c10" />
<img width="246" height="381" alt="可存证牌界面" src="https://github.com/user-attachments/assets/1fb14d89-52c7-40c4-9477-b2cc1604c6a1" />
<img width="246" height="381" alt="活动界面" src="https://github.com/user-attachments/assets/2ec07b48-e74e-4692-9d91-e53effb8eb47" />
<img width="246" height="381" alt="充值账单界面" src="https://github.com/user-attachments/assets/72b6ad88-7717-4407-b354-4bc2c2cfc9f4" />
<img width="246" height="381" alt="充币界面" src="https://github.com/user-attachments/assets/87193e5b-d26c-4c46-b9fc-318f61a53bc6" />
<img width="246" height="381" alt="保险界面" src="https://github.com/user-attachments/assets/23202125-00fb-4e72-a5a2-bd388749749b" />

## 📞 Contact Us

Telegram	@xuzongbin001

Backup Email	masterai918@gmail.com


## ⭐ Support Us
If you find this project helpful, please give us a Star ⭐—it is the greatest recognition for us!


https://api.star-history.com/svg?repos=masterai-top/Texas-Hold-em-Source-Code&type=Date
